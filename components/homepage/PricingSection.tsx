      ...
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <ScrollFadeIn>
          <div className="flex items-center justify-center gap-4 mb-12">
            <span
              className={`text-sm font-medium transition-colors ${!isYearly ? "text-[#e4e4ef]" : "text-[#8888a4]"}`}
            >
              Monthly
            </span>
            <button
            ...
            >
              <span
                className={`absolute top-[3px] left-[3px] w-5 h-5 rounded-full transition-all duration-200 ${
                  isYearly
                    ? "translate-x-5 bg-[#22c55e]"
                    : "translate-x-0 bg-[#8888a4]"
                }`}
              />
            </button>
            <span
          ...
          <ScrollFadeIn>
            <div className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-10 text-left transition-all duration-300 hover:-translate-y-1 max-sm:p-8">
              ...
              <ul className="flex flex-col gap-3.5 mb-8">
                {FREE_FEATURES.map((f) => (
                  ...
                  >
                    {f.included ? (
                      <Check
                        className="size-[18px] text-[#22c55e] shrink-0"
                        strokeWidth={2.5}
                      />
                    ) : (
              ...
              <Button
                variant="outline"
                asChild
                className="w-full border-[#1e1e2e] text-[#8888a4] hover:text-[#e4e4ef] hover:border-[#8888a4] bg-transparent"
              >
            ...
            <div className="bg-gradient-to-b from-blue-500/[0.06] to-[#12121a] border border-blue-500 rounded-xl p-10 text-left relative transition-all duration-300 hover:-translate-y-1 max-sm:p-8">
              ...
              <ul className="flex flex-col gap-3.5 mb-8">
                {PRO_FEATURES.map((text) => (
                  <li
                    key={text}
                    className="flex items-center gap-2.5 text-sm text-[#8888a4]"
                  >
                    <Check
                      className="size-[18px] text-[#22c55e] shrink-0"
                      strokeWidth={2.5}
              ...
              <Button
                asChild
                className="w-full bg-gradient-to-r from-blue-700 via-blue-600 to-blue-400 text-white border-0 hover:opacity-90"
              >
